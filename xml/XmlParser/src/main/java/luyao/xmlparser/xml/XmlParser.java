...
import luyao.xmlparser.xml.bean.chunk.*;
...
import static luyao.xmlparser.utils.Reader.log;
...
public class XmlParser {
        ...
        try {
            ...
            for (int i = 1; i <= stringCount; i++) { // 没有读最后一个字符串
                ...
                if (i == stringCount) {
                    ...
                    string = new String(reader.readOrigin(lastStringLength));
                    reader.skip(2);
                } else {
                    reader.skip(2);
    ...
    private void parseStartTagChunk() {
        ...
        try {
            ...
            for (int i = 0; i < attributeCount; i++) {
                ...
                Attribute attribute = new Attribute(namespaceUri, nameAttr, valueStr, type, data);
                attributes.add(attribute);
            }
            StartTagChunk startTagChunk = new StartTagChunk(namespaceUri, name, attributes);
    ...
    private void parseEndTagChunk() {
        ...
        try {
            ...
            EndTagChunk endTagChunk = new EndTagChunk(namespaceUri, name);
    ...
    private void generateXml() {
        Xml xml = new Xml(stringChunkList, null, chunkList,nameSpaceMap);
